class UserController < ApplicationController
  ...
  def login
    ...
    if session[:user] = User.authenticate(params[:user][:login], params[:user][:password])
      ...
      #TranscriptsControllerのlistアクションを呼び出す
      redirect_to :controller => 'transcripts', :action => 'list'
    else
  ...
  def change_password
    ...
    if do_change_password_for(@user)
      #パスワードを変更したら、TranscriptsControllerのlistアクションを呼び出す
      redirect_to :controller => 'transcripts', :action => 'list'
    end
  end
      ...
      begin
        User.transaction(user) do
          ...
          if user.save
            ...
            flash[:warning] = 'パスワードの変更に失敗しました。もう一度変更してください。'
            return false
          end
        end
      rescue
